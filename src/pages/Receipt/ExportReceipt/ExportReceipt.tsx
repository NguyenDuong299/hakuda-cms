...
export default function ExportReceiptManagement() {
  ...
  const [selected, setSelected] = useState<ExportReceipts | null>(null);
  ...
  const fetchExportReceipt = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/export-receipts`, {
        params: {
          page,
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await axios.delete(`${API_URL}/api/export-receipts/${id}`);
      fetchExportReceipt();
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        {exportReceipt.length > 0 ? (
          ...
          <Blank tittle="Không có dữ liệu" description="Không tìm thấy dữ liệu xuất hàng từ hệ thống!" />
        )}
      </div>
      {showConfirm && (
        <Confirm
          title="Bạn chắc chắn muốn xóa?"
          message="Sẽ không thể phục hồi sau khi xoá."
          onConfirm={() => {
            if (selected) {
